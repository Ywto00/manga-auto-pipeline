...
const { getDefaultState } = require('../../services/settings-service');
...
async function runSearchSettingsFlow(deps) {
  ...
  try {
    ...
    while (true) {
      ...
      if (action.act === 'save') {
        ui.separator();
        const { computeFinalConfig } = require('../../services/settings-service');
        const next = computeFinalConfig(cfg, state);
